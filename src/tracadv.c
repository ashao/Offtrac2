...
//	printf("cs137: %e\n",tr[3][0][64][147]); //ashao
  ...
  /* beginning of itt loop */
	...
	printf(" Advection iteration %d/%d\n",itt+1,NUM_ADV_ITER);
      /* big loop over k	 */
      for (k=0;k<NZ;k++)
...
# ifdef USE_CALC_H
		...
		if (h[k][i][j] < 0.0)
		  printf("tracadv l 796 - h[%d][%d][%d] = %g\n", k,i,j,
			 h[k][i][j]);	 
# else
//		fract1=1; fract2=0; //ashao: 03/18/2011
...
/*  increment.                                                        */
  ...
  double dTr[NXMEM][NYMEM];     /* The change in a tracer's concen-   */
                                /* tration, in units of concentration.*/
  ...
  for (k=0;k<NZ;k++) {
      ...
      for (i=X0;i<=nx;i++) {
	  for (j=Y1;j<=ny;j++) {
	      khdt_rem_x[i][j] = dt*KHTR*DYu(i,j)*IDXu(i,j)*umask[i][j];
	  }
      }
      for (i=X1;i<=nx;i++) {
	  for (j=Y0;j<=ny;j++) {
	      khdt_rem_y[i][j] = dt*KHTR*DXv(i,j)*IDYv(i,j)*vmask[i][j];
	  }
      }
    ...
    while (domore_k) {
	  ...
	  for (j=Y0;j<=ny;j++) {
          ...
          khdt_rem_y[i][j] -= C0;
         Coef_y[i][j] = C0*2.0*h[k][i][j]*h[k][i][j+1] /
	    (h[k][i][j]+h[k][i][j+1]);
        }
      }
      ...
      for (j=Y1;j<=ny;j++) {
        for (i=X0;i<=nx;i++) {
          ...
          khdt_rem_x[i][j] -= C0;
          Coef_x[i][j] = C0*2.0*h[k][i][j]*h[k][i+1][j] /
	    (h[k][i][j]+h[k][i+1][j]);
        }
        for (i=X1;i<=nx;i++) Ihdxdy[i][j] = IDXDYh(i,j) / h[k][i][j];
      }
...
/*	      Ihdxdy[nx+1][j] = Ihdxdy[2][j];
	      Ihdxdy[nx+2][j] = Ihdxdy[3][j];
	      Ihdxdy[0][j]    = Ihdxdy[nx-1][j];
	      Ihdxdy[1][j]    = Ihdxdy[nx][j];    */
	  }
...
//	      Ihdxdy[ii][ny+1] = Ihdxdy[i][ny];
//	      Ihdxdy[ii][ny+2] = Ihdxdy[i][ny-1];
	  }
//BX-e
	  for (i=X1;i<=nx;i++) {
	      for (j=Y1;j<=ny;j++) {
...
//BX-a
//BX   add re-entrace values when iteration loop is done here
		/* zonal re-entrance		*/
