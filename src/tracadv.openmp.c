...
void tracer(int itts)
{
	...
	double slope[NXMEM+NYMEM][run_parameters.numtracers]; /* The concentration slope per grid */
	/* point in units of concentration (nondim.). */
	double fluxtr[NXMEM+NYMEM][run_parameters.numtracers];/* The flux of tracer across a      */
			...
			for (k=0;k<NZ;k++)
			{
				...
				for (j=Y1;j<=ny;j++) {
					...
					for (i=X0;i<=nx;i++) {
						if (uhr[k][i][j] == 0.0) {
							uhh[i] = 0.0;
							for (m=0;m<run_parameters.numtracers;m++) fluxtr[i][m] = 0.0;
						}
				...
				for (i=X1;i<=nx;i++) {
					...
					for (j=Y0;j<=ny;j++) {
						if (vhr[k][i][j] == 0.0) {
							vhh[j] = 0.0;
							for (m=0;m<run_parameters.numtracers;m++) fluxtr[j][m] = 0.0;
						}
...
#pragma omp for  private(i,j,k)
			...
			//yanxu	    for (m=0;m<run_parameters.numtracers;m++) {
...
static void diffuse_tracer()
{
	...
	printf("DIFFUSING TRACER - diapycnal dt=%f\n",dt);
...
static void tracer_hordiff(void)
{
	...
	printf("DIFFUSING TRACER - isopycnal dt=%f\n",dt);
